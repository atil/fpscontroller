...
namespace Sabresaurus.SabreCSG
{
    public class DrawEditor : Tool
    {
		...
		enum DrawMode 
		{ 
			None, // Drawing is not active or has been cancelled
			Ambiguous, // They've started drawing but we're not sure if it's a rectangle or point sequence
			RectangleBase, // They're drawing a rectangle through a click and drag
			PolygonBase // They're defining a polygon by clicking a sequence of points
		};
		...
		PolygonRaycastHit? CalculateHitPolygon(Vector2 currentPosition)
		{
		...
		void OnMouseMoveSelectHeight(SceneView sceneView, Event e)
		{
